...
function calculateQuantities(ingredientList, numOfLayers) {
  const object = {}
  if (ingredientList.find((x) => x === 'sugar')) {
    object.sugar = 100 * numOfLayers
  } else {
    object.sugar = 0
  } // if(ingredients.hasOwnProperty(ingredient))
  if (ingredientList.find((x) => x === 'eggs')) {
...
function improveRecipe(ingredients, numOfPortions) {
  const object = {}
  for (const ingredient in ingredients) {
    object[ingredient] = ingredients[ingredient] * numOfPortions
  }
  return object
}
